...
ASR_MODEL_PATH = "/mnt/d/storage/models/qwen-asr-1.7b"
sample = load_from_disk('data/sada').select(range(1))[0]
...
def run_streaming_case(asr: Qwen3ASRModel, wav16k: np.ndarray, step_ms: int) -> None:
    ...
    print(f"\n\n[final] language={state.language!r} text={state.text!r}\n\n")
    print(f'Ground truth: {sample["text"]!r}')
